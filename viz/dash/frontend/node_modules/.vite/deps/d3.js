...
function disjoint(values, other) {
  const iterator = other[Symbol.iterator](), set5 = new InternSet();
  for (const v2 of values) {
    if (set5.has(v2)) return false;
    let value, done;
...
function superset(values, other) {
  const iterator = values[Symbol.iterator](), set5 = /* @__PURE__ */ new Set();
...
Dispatch2.prototype = dispatch2.prototype = {
  ...
  on: function(typename, callback) {
    ...
    if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set4(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set4(_[t], typename.name, null);
    }
    return this;
  },
  copy: function() {
...
function simulation_default(nodes) {
  var simulation, alpha = 1, alphaMin = 1e-3, alphaDecay = 1 - Math.pow(alphaMin, 1 / 300), alphaTarget = 0, velocityDecay = 0.6, forces = /* @__PURE__ */ new Map(), stepper = timer(step), event = dispatch_default3("tick", "end"), random = lcg_default();
  if (nodes == null) nodes = [];
  function step() {
    tick();
